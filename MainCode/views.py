...
def busmanagement_add(request,):
    ...
    if request.method == "POST":
        ...
        ob.RouteId = obb
        ob.save()
        big_code = pyqrcode.create(str(ob.BusId), error='L', version=27, mode='binary')
        qrs ="C:/Users/00000/Desktop/LGEW-main/MainCode/static/qr_code/" + str(ob.BusId) + ".png"
...
def passenger(request):
    ...
    if request.method == "POST":
        ...
        ob1 = PassengerBooking.objects.filter(BusId=ob.BusId).aggregate(Count('PassengerId'))
...
def userlogin(request):
    ...
    try:
        ob=Login.objects.get(Username=username,Password=passwd,Type='conductor')
        print(ob)
        ...
        try:
            ob=Login.objects.get(Username=username,Password=passwd,Type='user')
            print(ob)
...
def selectroute(request):
    ...
    res1=[]
    for i in ob:
        r={"rid":i.RouteId,"start_stop":i.StartingStop,"end_stop":i.EndingStop}
        res1.append(r)
...
def viewroutes(request):
    ...
    res1=[]
    for i in obb:
        r={"bregno":i.BusRegisterNUmber,"seatcapasity":i.SeatCapacity,"bid":i.BusId}
...
def androidbustime(request):
    ...
    res1=[]
    for i in obb:
        r={"stop":i.StopId.stop,"ticketchange":i.StopId.TicketCharge,"time":i.Time,"stopid":i.StopId.StopId}
        res1.append(r)
    r = json.dumps(res1)
    print(r)
    return HttpResponse(r)
...
def Bank1(request):
    ...
    obp=Passenger.objects.get(UserId=uid)
    obbank=Bank.objects.filter(PassangerId=obp,AccountNumber=cno,IFSE_code=pin)
    if len(obbank)==0:
        res1={"result":"invalid"}
        r = json.dumps(res1)
        print(r)
